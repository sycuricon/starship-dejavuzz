...
class IbexCoreBlackbox(
    ...
    extends BlackBox(
        Map(
            ...
            "ICache" -> IntParam(iCache),
            "ICacheECC" -> IntParam(iCacheECC),
            "ICacheScramble" -> IntParam(iCacheScramble),
            "BRANCH_PREDICTOR" -> IntParam(branchPredictor),
            "DbgTriggerEn" -> IntParam(dbgTriggerEn),
            "SecureIbex" -> IntParam(secureIbex),
    ...
    with HasBlackBoxPath
{
    val io = IO(new Bundle {
        ...
        val fetch_enable_i = Input(UInt(4.W))
